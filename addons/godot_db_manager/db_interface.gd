...
func on_save_database_btn_pressed():
	...
	for idx in range(0, m_tables.size()):
		database[str(m_tables[idx].get_name())] = {}
		for jdx in range(0, m_tables[idx].get_props_count()):
			database[m_tables[idx].get_name()][str(m_tables[idx].get_prop_name(jdx))] = m_tables[idx].get_data_at(jdx)

	var save_file = File.new()
	save_file.open(m_current_db_name, File.WRITE)
	for each in database:
#		save_file.store_string("Table: " + str(each))
		save_file.store_line(to_json(database[each]))
	save_file.close()
